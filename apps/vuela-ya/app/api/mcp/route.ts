...
async function handleMcpRequest(req: NextRequest | Request): Promise<Response> {
  // Handle CORS preflight for Claude.ai / Web Clients
  ...
  // Handle GET health check and discovery probes from Claude / browsers
  if (req.method === 'GET') {
    return new Response(
      JSON.stringify({
        status: 'ok',
        name: 'vuela-ya-mcp',
        version: '1.0.0',
        description: 'Vuela Ya Colombian Domestic Airline MCP Server',
